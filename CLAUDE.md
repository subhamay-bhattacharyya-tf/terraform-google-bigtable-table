...
This is a **Terraform module** that creates and manages a single `google_bigtable_table` resource on GCP. The entire public interface is one input variable (`bigtable_table_config`) and outputs for table attributes (id, name, project, instance_name, column_family, deletion_protection). This will be used as a GitHub Repository Template. The actual modules with me implemented seperately.
...
cd examples/bigtable/basic && terraform init -backend=false && terraform validate

# Run Terratest integration test (requires GCP auth + GOOGLE_CLOUD_PROJECT env var)
cd test && go test -v -timeout 30m -run TestBigtableTableBasic ./bigtable_table_basic_test.go ./helpers_test.go
...
├── main.tf            # Single google_bigtable_table resource
├── variables.tf       # bigtable_table_config object variable with all validations
├── outputs.tf         # Bigtable table attribute outputs
├── versions.tf        # Terraform >= 1.3.0, google provider >= 7.23.0
├── examples/
│   └── bigtable/basic/  # Reference usage; CI validates this separately
└── test/
    ├── bigtable_table_basic_test.go   # Terratest: creates real Bigtable table, asserts outputs, destroys
...
The module uses a single structured `bigtable_table_config` object rather than flat variables. All validation (naming rules, project ID format, column family settings, deletion protection values) lives in `variables.tf`.
...
2. **examples-validate** — `init` + `validate` on `examples/bigtable/basic` (needs step 1)
...
- `test/helpers_test.go` contains AWS S3 helpers; `test/go.mod` references `terraform-aws-s3`. These are unused by the Bigtable test and should be replaced with Bigtable-specific helpers when adding new tests.
